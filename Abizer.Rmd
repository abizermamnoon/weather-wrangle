...
```{r echo = FALSE}
MainStates <- map_data("state")
ggplot() + coord_quickmap() + geom_map(data = Temps, aes(map_id = state, fill = wind_speed), map = MainStates) + expand_limits(x = MainStates$long, y = MainStates$lat) + ggtitle(" 2016 Average Wind Speed in US States") + scale_fill_distiller(palette = "Gray",direction = 1)
```
