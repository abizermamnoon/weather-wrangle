...
library(ggplot2)
library(dplyr)
Consistent <- MonthlyWeather %>% select(month,year,avgTmpDiffF)  %>% group_by(month) %>% summarize(mn = mean(avgTmpDiffF)) %>% arrange(mn)
Consistent %>% ggplot(aes(x = month, y = mn)) + geom_bar(stat = 'identity') + coord_flip() 
```
```{r}
library(ggplot2)
library(dplyr)
Sunny <- MonthlyWeather %>% select(month,year,numSunDay)  %>% group_by(month) %>% summarize(mn = mean(numSunDay)) %>% arrange(mn)
Sunny %>% ggplot(aes(x = month, y = mn)) + geom_bar(stat = 'identity') + coord_flip()
```
